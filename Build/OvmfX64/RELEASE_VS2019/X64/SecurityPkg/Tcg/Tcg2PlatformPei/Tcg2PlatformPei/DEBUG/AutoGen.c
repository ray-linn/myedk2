...
GLOBAL_REMOVE_IF_UNREFERENCED GUID gEfiCallerIdGuid = {0x47727552, 0xA54B, 0x4A84, {0x8C, 0xC1, 0xBF, 0xF2, 0x3E, 0x23, 0x96, 0x36}};
...
GLOBAL_REMOVE_IF_UNREFERENCED CHAR8 *gEfiCallerBaseName = "Tcg2PlatformPei";
...
#define _PCD_VALUE_PcdRandomizePlatformHierarchy  1U
GLOBAL_REMOVE_IF_UNREFERENCED const BOOLEAN _gPcd_FixedAtBuild_PcdRandomizePlatformHierarchy = _PCD_VALUE_PcdRandomizePlatformHierarchy;
  ...
  )
{
  ...
  Status = Tpm2DeviceLibConstructor ();
  ASSERT_RETURN_ERROR (Status);
...
{
  return Tcg2PlatformPeiEntryPoint (FileHandle, PeiServices);
}
